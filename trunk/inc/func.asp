<%
    ...
    function get_art_list_by_page(tid, pgsize)
        ...
        ' 获取结果
        dim tmp_list : set tmp_list = Easp.db.GetPageRecord(0, array("content", condition, "time desc"))
    ...
    function get_art_list(tid, num, pro)
        ...
        dim tmp_list
        set tmp_list = Easp.db.GR("content:" & num, condition, "time desc")
    ...
    function get_flash(tid, width, height)
        dim tmp_str
        tmp_str = "<object type=""application/x-shockwave-flash"" id=""XadillaX_Flash"" data=""" & Root & "images/bcastr4.swf?xml=" & Root & "getdata.asp?action=flash_xml&tid=" & tid & """ width=""" & width & """ height=""" & height & """>"
        tmp_str = tmp_str & "<param name=""movie"" value=""" & Root & "images/bcastr4.swf?xml=" & Root & "getdata.asp?action=flash_xml&tid=" & tid & """ />"
