...
<table class="mytable" border="0" cellspacing="0" cellpadding="0">
    <tr class="table_title">
        ...
        <td width="11%" valign="middle">用户名</td>
        <td width="48%" valign="middle">操作内容</td>
        <td width="18%" valign="middle">操作时间</td>
    ...
    <tr class="table_row <%=iif(i mod 2 = 1, "table_row_clr_1", "table_row_clr_2")%>">
        <td><%=rs("lid")%></td>
        <td><%=rs("username")%></td>
        <td><div style="max-height: 60px; overflow-x: auto; overflow-y: auto; color: <%=iif(rs("right") = true, "#00cc00", " #ff0000")%>; padding: 5px 5px 5px 5px; border: #ddd 1px solid; background: #fff; margin: 2px 2px 2px 2px;"><%=rs("logtext")%></div></td>
        <td><%=rs("time")%></td>
        <td><%=rs("ip")%></td>
    </tr>
    <%
